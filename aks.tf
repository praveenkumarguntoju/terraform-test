...
resource "azurerm_private_dns_zone" "aks" {
  name                = "privatelink.${var.azure-region}.azmk8s.io"
  resource_group_name = azurerm_resource_group.rg.name
}

resource "azurerm_private_dns_zone_virtual_network_link" "aks1" {
  name                  = "${var.solution-name}-${var.environment}-pdzvnl-aks-001"
  resource_group_name   = azurerm_resource_group.rg.name
...
resource "azurerm_private_dns_zone_virtual_network_link" "aks2" {
  name                  = "${var.solution-name}-${var.environment}-pdzvnl-aks-002"
  resource_group_name   = azurerm_resource_group.rg.name
  private_dns_zone_name = azurerm_private_dns_zone.aks.name
...
resource "azurerm_user_assigned_identity" "aks" {
  name                = "${var.solution-name}-${var.environment}-id-aks-001"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
}

resource "azurerm_user_assigned_identity" "pod" {
  name                = "${var.solution-name}-${var.environment}-id-pod-001"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
}
...
resource "azurerm_kubernetes_cluster" "aks" {
  name                       = "${var.solution-name}-${var.environment}-aks"
  location                   = azurerm_resource_group.rg.location
  resource_group_name        = azurerm_resource_group.rg.name
